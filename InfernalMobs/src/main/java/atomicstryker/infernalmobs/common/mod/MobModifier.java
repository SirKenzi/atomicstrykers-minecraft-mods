...
public abstract class MobModifier {
    ...
    protected abstract MobModifierType getMonsterModifierType();
    ...
    public String getModName(){
        return this.getMonsterModifierType().getId();
    };
    ...
    private void increaseMaxHealthForMobIfNeeded(LivingEntity mob) {
        ...
        if (storedMaxHealth < 1F) {
            actualMaxHealth = (float) (ConfigStore.getMobClassMaxHealth(mob) * getModSize() * ConfigStore.getConfig().getModHealthFactor());
    ...
    protected List<String> getModNamePrefix() {
        return this.getMonsterModifierType().getPrefixes();
    }

    protected List<String> getModNameSuffix() {
        return this.getMonsterModifierType().getSuffixes();
    }
    ...
    public String getEntityDisplayName(LivingEntity target) {
        if (bufferedEntityName == null) {
            ...
            if (mod.getModNamePrefix() != null) {
                modprefix = mod.getModNamePrefix().get(target.getRandom().nextInt(mod.getModNamePrefix().size()));
            ...
            if (size > 1) {
                ...
                if (mod.getModNameSuffix() != null) {
                    String pickedSuffix = mod.getModNameSuffix().get(target.getRandom().nextInt(mod.getModNameSuffix().size()));
